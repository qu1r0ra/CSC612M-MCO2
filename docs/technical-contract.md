...
3. **Empirical unbiasedness (course subset)**:
   ...
   - **Full expectation suite (paper extension)**, `unbiasedness.py`, spec [qu1r0ra/CSC612M-MCO2-Paper#21](https://github.com/qu1r0ra/CSC612M-MCO2-Paper/issues/21):
     ...
     - Target: the course subset keeps the $x_i$ rule above. The full suite compares the mean with the expectation of the implemented quantizer instead, because FP32 rounding of $a_i$ and of the decoded levels can move that expectation off $x_i$ by more than the $5\sigma$ bound on arbitrary inputs. In FP32, $a_i = \min((|x_i|/\text{scale}) \cdot s, s)$, $l_i = \lfloor a_i \rfloor$, $p_i = a_i - l_i$. The codec rounds up when the 32-bit word is below $\lfloor \text{fl}_{32}(p_i \cdot 2^{32}) \rfloor$, so the realised probability is $q_i = \lfloor \text{fl}_{32}(p_i \cdot 2^{32}) \rfloor / 2^{32}$. With $\text{dec}(k) = \text{fl}_{32}(\text{fl}_{32}(k/s) \cdot \text{scale})$, signed like $x_i$: $E_i = (1-q_i)\,\text{dec}(l_i) + q_i\,\text{dec}(l_i+1)$ and $\Delta_i = |\text{dec}(l_i+1) - \text{dec}(l_i)|$.
     - Acceptance rule: where $q_i > 0$, $|\bar{y}_i - E_i| \le 5 \cdot \Delta_i \cdot \sqrt{q_i(1-q_i)/T}$; where $q_i = 0$, $\bar{y}_i = \text{dec}(l_i)$ exactly. The suite fails if any element fails or the backends differ.
