...
- **Timing boundaries**:
  ...
  - `resident-graph` (CUDA-only): measures device input to device packed bytes using a pre-captured CUDA Graph (1 memset + 12 kernels) executed via `cudaGraphLaunch` on the benchmark stream, followed by stream synchronization. Capturing and instantiating occurs once outside timing and is recorded separately (`capture_ms` / `capture_and_instantiate_ms`); individual kernel stages (K1-K3) are absent for this boundary.
...
- **Invocation identifiers**: Repetition `r` (0-indexed) uses invocation identifier `base + r`; warmup `w` uses `base + reps + w`. Identifiers that would exceed $2^{32}-1$ fail before timing begins. (For `resident-graph`, the captured graph node parameters fix the base invocation sequence for the repetition executions while the separate uncaptured preflight record run outputs the exact base record).
