...
- **Timing boundaries**:
  ...
  - `gpu-origin` with `--backend cuda`: the input is uploaded to the device once, outside timing. Each repetition runs K1, K2, and K3 on the resident input, then copies the packed payload and scale to the host with `cudaMemcpyAsync` on the benchmark stream, synchronizing the CUDA device before stopping the monotonic clock. `d2h_ms` is the event time of those copies.
