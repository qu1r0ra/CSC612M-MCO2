...
- **Trial order.** Above four paths the driver uses a Williams design instead of all orderings, since 9! orderings cannot run. With nine paths one design is 18 orders, so `--trials` must be a multiple of 18, and an extension run, pilots included, must pass it explicitly because the default 24 is rejected; the extension default for #25 is set with that sweep. The manifest (version 3.1) records `paths`, `trial_design`, and `transfer_policies`.
